...
public class GameStatus {
   ...
   *          - the number of asteroids that have been destroyed
   */
  ...
  /**
   * Updates the number of asteroids that are on the screen at a given time
  ...
  /**
   * Updates the number of enemy ships in the screen in a given time, depending
   * on the level.
   * 
   * @param level
   *          - the level the player is in
  ...
  /**
   * Updates the level number depending on the player's game status (targets
   * destroyed, points, etc.)
   */
  public synchronized void updateLevel() {
    if (GameSettings.CHANGE_LEVEL_WITH_POINTS)
      ...
      setLevel((int) (getEnemyShipsDestroyed() + getAsteroidsDestroyed())
          / GameSettings.TARGETS_TO_NEXT_LEVEL + 1);
  }
   ...
   * 
   * @param gameLogic
   *          - the game logic that will be used
   */
